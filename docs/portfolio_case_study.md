...
> Kaggleの恒星分類コンペで、公開OOF/test予測と自作モデルを統合する再現可能なアンサンブル検証基盤を構築しました。単純な提出ファイルのコピーではなく、OOFベースの重み最適化、形式の異なる予測ファイルの自動検出、stacking / hard vote / class-prior probeの比較、Public LBとの乖離分析を行いました。最終的に公開予測を含む構成でPublic LB 0.97044、自力モデル中心の構成でPublic LB 0.96883を達成しました。
